...
def get_spacy_model():
    return spacy.load("en_core_web_lg")  # Load the English Model
...
def generate_quiz(pdf_name, answers):
    ...
    bucket = storage_client.bucket("pdf2quiz")
        ...
        for answer in answers.keys():
            ...
            for _, target in targets.iterrows():
                ...
                blob.upload_from_string(
                    encoded_image.tobytes(), content_type="image/png"
                )
                original_image_name = blob.public_url
                _, encoded_image = cv2.imencode(".png", masked_image)
                blob = bucket.blob(masked_image_name)
                blob.upload_from_string(
                    encoded_image.tobytes(), content_type="image/png"
                )
                masked_image_name = blob.public_url
